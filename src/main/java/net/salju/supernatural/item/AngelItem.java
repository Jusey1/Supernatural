...
public class AngelItem extends Item {
	...
	public InteractionResult useOn(UseOnContext context) {
		...
		if (direct == Direction.DOWN) {
			...
			AABB nums = SupernaturalModEntities.ANGEL.get().getDimensions().makeBoundingBox(veccy.x(), veccy.y(), veccy.z());
			if (world.noCollision((Entity) null, nums) && world.getEntities((Entity) null, nums).isEmpty()) {
				if (world instanceof ServerLevel server) {
					Consumer<AngelEntity> consumer = EntityType.createDefaultStackConfig(server, stack, context.getPlayer());
					AngelEntity angel = SupernaturalModEntities.ANGEL.get().create(server, stack.getTag(), consumer, pos, MobSpawnType.SPAWN_EGG, true, true);
					if (angel == null) {
						return InteractionResult.FAIL;
					}
