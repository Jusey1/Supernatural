		...
		if (direct == Direction.DOWN) {
			...
			if (world.noCollision((Entity) null, nums) && world.getEntities((Entity) null, nums).isEmpty()) {
				if (world instanceof ServerLevel lvl) {
					Consumer<Angel> consumer = EntityType.createDefaultStackConfig(lvl, stack, context.getPlayer());
					Angel angel = SupernaturalMobs.ANGEL.get().create(lvl, stack.getTag(), consumer, pos, MobSpawnType.SPAWN_EGG, true, true);
					if (angel == null) {
						return InteractionResult.FAIL;
					}
					...
					angel.yBodyRotO = rot;
					angel.yHeadRotO = rot;
					lvl.addFreshEntityWithPassengers(angel);
