...
public class GothicHelmetItem extends DyeableArmorItem {
	...
	public void initializeClient(Consumer<IClientItemExtensions> consumer) {
		consumer.accept(new IClientItemExtensions() {
			@Override
			public HumanoidModel<?> getHumanoidArmorModel(LivingEntity target, ItemStack stack, EquipmentSlot armorSlot, HumanoidModel<?> df) {
				if (isKobold(target)) {
	...
	private static boolean isKobold(LivingEntity target) {
		return target.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("forge:kobolds")));
	}
}