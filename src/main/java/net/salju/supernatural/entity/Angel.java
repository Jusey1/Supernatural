...
public class Angel extends Mob {
	public Angel(PlayMessages.SpawnEntity packet, Level world) {
		this(SupernaturalMobs.ANGEL.get(), world);
	}
	...
	public void baseTick() {
		...
		if (!world.isClientSide() && this.isAlive() && this.isEffectiveAi() && (SupernaturalConfig.FURIA.get() == true)) {
			for (LivingEntity target : world.getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(12.0D))) {
				...
				} else if (SupernaturalHelpers.isVampire(target) || (target.getMobType() == MobType.UNDEAD) || target.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("supernatural:is_vampire")))) {
					if (!target.isOnFire() && !(target instanceof Spooky) && !target.fireImmune()) {
