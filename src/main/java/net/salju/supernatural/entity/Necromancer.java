...
public class Necromancer extends SpellcasterIllager {
	public Necromancer(PlayMessages.SpawnEntity packet, Level world) {
		this(SupernaturalMobs.NECROMANCER.get(), world);
	}
	...
	class NecroLifeStealSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
		public boolean canUse() {
			if (Necromancer.this.getTarget() != null) {
				LivingEntity target = Necromancer.this.getTarget();
				if (target.hasEffect(SupernaturalEffects.BLEEDING.get()) || (Necromancer.this.getHealth() > (Necromancer.this.getMaxHealth() * 0.45))) {
					return false;
				} else {
		...
		protected void performSpellCasting() {
			...
			target.addEffect(new MobEffectInstance(SupernaturalEffects.BLEEDING.get(), 380, 0));
	...
	class NecroSummonSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
		...
		protected void performSpellCasting() {
			...
			for (int i = 0; i < 3; ++i) {
				...
				Vex vex = EntityType.VEX.create(Necromancer.this.level());
				vex.moveTo(blockpos, 0.0F, 0.0F);
				vex.finalizeSpawn(serverlevel, Necromancer.this.level().getCurrentDifficultyAt(blockpos), MobSpawnType.MOB_SUMMONED, (SpawnGroupData) null, (CompoundTag) null);
				vex.setOwner(Necromancer.this);
				vex.setBoundOrigin(blockpos);
				vex.setLimitedLife(20 * (30 + Necromancer.this.random.nextInt(90)));
