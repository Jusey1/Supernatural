...
public class RitualCompassState extends NeedleDirectionHelper {
    ...
    private float getTarget(ItemOwner target, long time, BlockPos pos) {
    ...
    private static boolean isValid(ItemOwner target, @Nullable GlobalPos pos) {
        return pos != null && pos.dimension().equals(target.level().dimension()) && !(pos.pos().distToCenterSqr(target.position()) < 1.0E-5F);
    }

    private static double getAngle(ItemOwner target, BlockPos pos) {
        return Math.atan2(Vec3.atCenterOf(pos).z() - target.position().z(), Vec3.atCenterOf(pos).x() - target.position().x()) / (float) (Math.PI * 2);
    }

    private static float getRotY(ItemOwner target) {
