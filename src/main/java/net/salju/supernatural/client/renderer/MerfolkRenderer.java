...
public class MerfolkRenderer extends MobRenderer<Merfolk, SupernaturalRenderState, MerfolkModel<SupernaturalRenderState>> {
    ...
    public void extractRenderState(Merfolk target, SupernaturalRenderState state, float f1) {
        ...
        state.type = this.getMerfolkType(target);
    }
    ...
    public String getMerfolkType(Merfolk target) {
        if (target.isCaptain()) {
            return "diamond";
        } else if (target.isSeadog()) {
            return "emerald";
        }
        return "amethyst";
    }
